    ...
    const task_done = (task, task_tool) => {
        finished_task_container = document.getElementsByClassName('container')[1];
        task.removeChild(task_tool);

        finished_task_container.append(task);

        let key = task.getAttribute("data-key");
        let task_obj = {
    ...
    const task_edit = (task, edit_button) => {
    ...
    const finish_edit = (task, edit_button) => {
    ...
    const task_finished_delete = (task) => {
        key = task.getAttribute("data-key");
